...
mock_cfg="$curr_dir/altiscale-zeppelin-centos-6-x86_64.cfg"
...
if [ "x${ZEPPELIN_VERSION}" = "x" ] ; then
  echo >&2 "fail - ZEPPELIN_VERSION can't be empty"
  exit -8
else
  echo "ok - ZEPPELIN_VERSION=$ZEPPELIN_VERSION"
fi

if [ "x${ZEPPELIN_PLAIN_VERSION}" = "x" ] ; then
  echo >&2 "fail - ZEPPELIN_PLAIN_VERSION can't be empty"
  exit -8
else
  echo "ok - ZEPPELIN_PLAIN_VERSION=$ZEPPELIN_PLAIN_VERSION"
fi
...
# should switch to WORKSPACE, current folder will be in WORKSPACE/zeppelin due to 
# hadoop_ecosystem_component_build.rb => this script will change directory into your submodule dir
# WORKSPACE is the default path when jenkin launches e.g. /mnt/ebs1/jenkins/workspace/zeppelin_build_test-alee
# If not, you will be in the $WORKSPACE/zeppelin folder already, just go ahead and work on the submodule
# The path in the following is all relative, if the parent jenkin config is changed, things may break here.
pushd `pwd`
cd $WORKSPACE/zeppelin

# Manual fix Git URL issue in submodule, safety net, just in case the git scheme doesn't work
# sed -i 's/git\@github.com:Altiscale\/zeppelin.git/https:\/\/github.com\/Altiscale\/zeppelin.git/g' .gitmodules
# sed -i 's/git\@github.com:Altiscale\/zeppelin.git/https:\/\/github.com\/Altiscale\/zeppelin.git/g' .git/config
if [ "x${ZEPPELIN_BRANCH_NAME}" = "x" ] ; then
  echo "error - ZEPPELIN_BRANCH_NAME is not defined, even though, you may checkout the code from hadoop_ecosystem_component_build, this does not gurantee you have the right branch. Please specify the BRANCH_NAME explicitly. Exiting!"
  exit -9
fi
echo "ok - switching to zeppelin branch $ZEPPELIN_BRANCH_NAME and refetch the files"
...
# zeppelin is located at $WORKSPACE/zeppelin
# tar cvzf $WORKSPACE/zeppelin.tar.gz zeppelin

# Looks like this is not installed on all machines
# rpmdev-setuptree
mkdir -p $WORKSPACE/rpmbuild/{BUILD,BUILDROOT,RPMS,SPECS,SOURCES,SRPMS}/
cp "$zeppelin_spec" $WORKSPACE/rpmbuild/SPECS/zeppelin.spec
pushd $WORKSPACE/
...
tar --exclude .git --exclude .gitignore -czf alti-zeppelin.tar.gz alti-zeppelin
if [ -f "$maven_settings" ] ; then
...
# Build alti-maven-settings RPM separately so it doesn't get exposed to zeppelin's SRPM or any external trace
...
sed -i "s/ZEPPELIN_USER/$ZEPPELIN_USER/g" "$WORKSPACE/rpmbuild/SPECS/zeppelin.spec"
...
SCALA_HOME=$SCALA_HOME rpmbuild -vv -bs $WORKSPACE/rpmbuild/SPECS/zeppelin.spec --define "_topdir $WORKSPACE/rpmbuild" --buildroot $WORKSPACE/rpmbuild/BUILDROOT/

if [ $? -ne "0" ] ; then
  echo "fail - zeppelin SRPM build failed"
  cleanup_secrets
  exit -98
fi

stat "$WORKSPACE/rpmbuild/SRPMS/alti-zeppelin-${ZEPPELIN_VERSION}-${ZEPPELIN_VERSION}-${ALTISCALE_RELEASE}.${BUILD_TIME}.el6.src.rpm"
rpm -ivvv "$WORKSPACE/rpmbuild/SRPMS/alti-zeppelin-${ZEPPELIN_VERSION}-${ZEPPELIN_VERSION}-${ALTISCALE_RELEASE}.${BUILD_TIME}.el6.src.rpm"
...
sed -i "s:ZEPPELIN_VERSION:$ZEPPELIN_VERSION:g" "$curr_dir/$mock_cfg_runtime"
...
# mock -vvv --configdir=$curr_dir -r altiscale-zeppelin-centos-6-x86_64.runtime --scrub=all
mock -vvv --configdir=$curr_dir -r altiscale-zeppelin-centos-6-x86_64.runtime --init

mock -vvv --configdir=$curr_dir -r altiscale-zeppelin-centos-6-x86_64.runtime --no-clean --no-cleanup-after --install $WORKSPACE/rpmbuild/RPMS/noarch/alti-maven-settings-1.0-1.el6.noarch.rpm

mock -vvv --configdir=$curr_dir -r altiscale-zeppelin-centos-6-x86_64.runtime --no-clean --rpmbuild_timeout=$build_timeout --resultdir=$WORKSPACE/rpmbuild/RPMS/ --rebuild $WORKSPACE/rpmbuild/SRPMS/alti-zeppelin-${ZEPPELIN_VERSION}-${ZEPPELIN_VERSION}-${ALTISCALE_RELEASE}.${BUILD_TIME}.el6.src.rpm

if [ $? -ne "0" ] ; then
  echo "fail - mock RPM build failed"
  cleanup_secrets
  # mock --configdir=$curr_dir -r altiscale-zeppelin-centos-6-x86_64.runtime --clean
  mock --configdir=$curr_dir -r altiscale-zeppelin-centos-6-x86_64.runtime --scrub=all
  exit -99
fi
